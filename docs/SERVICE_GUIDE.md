...
| **저장소 (이미지/영상)** | Storage → Buckets (images, videos) |
...
| **글 순서 변경** | 글 목록에서 드래그앤드롭으로 순서 변경 |
| **카테고리 관리** | 카테고리 관리 메뉴에서 추가/수정/삭제/드래그앤드롭 순서 변경 |
| **수상 내역 관리** | 수상 내역 관리 메뉴에서 추가/수정/삭제 |
| **사이트 정보 수정** | 사이트 콘텐츠 관리 메뉴에서 이름, 소개글, 연락처, 배경 영상 수정 |
